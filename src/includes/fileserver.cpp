    ...
    void FileServer::readyRead()
    {
        ...
        while (socket->bytesAvailable() > 0 || buffer->size() >= 16)
        {
            ...
            if ( ! optData.has_value())
            {
                ...
                {
                    optData = std::make_unique<String>(ip, size, name);
                    QObject::connect(optData.value().get(), &Data::dataReceived, this, &FileServer::stringReceived);
                }
                else //Create data::File and connect
                {
                    optData = std::make_unique<File>(ip, size, name, path);
                    QObject::connect(optData.value().get(), &Data::dataReceived, this, &FileServer::fileReceived);
                }
            }
